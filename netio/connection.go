...
func (con *tcpHAPConnection) Write(b []byte) (int, error) {    
    ...
    buffer.Write(b)
    encrypted, err := con.context.Encrypt(&buffer)
    if err != nil {
        fmt.Println("Encryption failed", err)
        // End communication
        err = con.connection.Close()
...
func (con *tcpHAPConnection) Read(b []byte) (int, error) {
    ...
    if con.decryptedBuffer == nil {
        ...
        decrypted, err := con.context.Decrypt(buffered)
        if err != nil {
            fmt.Println("Decryption failed", err)
            // End communication
            err = con.connection.Close()
        ...
        fmt.Println("Buffered", con)
    }
    fmt.Println("Reading", con)
    n, err := con.decryptedBuffer.Read(b)
    fmt.Println("..... done ", con)
    if n < len(b) || err == io.EOF {
        con.decryptedBuffer = nil
        fmt.Println("..... cleanup", con)
    }
...
func (l *TCPHAPListener) Accept() (c net.Conn, err error) {
    ...
    hapConn, err := &tcpHAPConnection{connection: con, context: NewContext()}, nil
    if err == nil {
    ...
    fmt.Println("Accepted new connection", hapConn)
    return hapConn, err
}