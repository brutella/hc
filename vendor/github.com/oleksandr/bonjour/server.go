...
func newServer(iface *net.Interface) (*Server, error) {
	...
	s := &Server{
		ipv4conn: ipv4conn,
		ipv6conn: ipv6conn,
		ttl:      75 * 60, // Default TTL of 75min
	}
...
func (s *Server) handleQuestion(q dns.Question, resp *dns.Msg) error {
	...
	case s.service.HostName:
		s.composeHostNameAnswers(resp)
	case s.service.ServiceTypeName():
		s.composeServiceTypeNameAnswers(resp, s.ttl)
	}
...
func (s *Server) composeBrowsingAnswers(resp *dns.Msg, ttl uint32) {
	...
	resp.Extra = append(resp.Extra, s.hostNameAnswers()[:]...)
}

func (s *Server) composeHostNameAnswers(resp *dns.Msg) {
	resp.Answer = append(resp.Answer, s.hostNameAnswers()[:]...)
}
...
func (s *Server) hostNameAnswers() []dns.RR {
	answers := []dns.RR{}
...
func (s *Server) composeServiceTypeNameAnswers(resp *dns.Msg, ttl uint32) {
	// From RFC6763
